...
class ExplainabilityFrequencyGenerator(Dataset):  # type: ignore[type-arg]
    def __init__(self, subjects: Tuple[str, ...], dataset: CauEEGDataset, frequency_band: str,
                 device: Optional[torch.device] = None, keep_band=True):
        super().__init__()
    ...
    @staticmethod
    def apply_bandpass_exclusion(data, dataset, freq_band: list, keep_band):
        """
        ...
        - freq_band: List with two elements [l_freq, h_freq] defining the band to be excluded.
         - keep_band: Boolean indicating whether to keep only the specified frequency band (True) or
                     exclude it (False).
        ...
        for i, s in enumerate(data):
            raw = mne.io.RawArray(s, info=dataset.get_eeg_info(), verbose=False)
            raw.filter(l_freq=freq_band[1], h_freq=freq_band[0], verbose=False, fir_design="firwin", method="iir")
            # print(np.arange(freq_band[0], freq_band[1]))
            # raw.notch_filter(freqs=np.arange(freq_band[0], freq_band[1]), fir_design="firwin", verbose=False)
