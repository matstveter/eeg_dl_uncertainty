...
class _InceptionModule(nn.Module):
    ...
    def __init__(self, in_channels: int, units: int, use_bottleneck: bool , max_kernel_size: int,
        ...
        # -------------------------------
        if use_bottleneck:
            self._input_conv = nn.Conv1d(in_channels, out_channels=units, kernel_size=1, padding="same", bias=False)
            out_channels = units
        else:
...
# Main module
# ---------------------------
class InceptionWide(BaseClassifier):
    ...
    def __init__(self, **kwargs):
        ...
        self._fc_layer_age = nn.Linear(in_features=output_channels + 1, out_features=16)

        self._batch = nn.BatchNorm1d(16)
        self._fc_layer_last = nn.Linear(in_features=16, out_features=num_classes)
        self._act = nn.ReLU()
        self._dropout_last = nn.Dropout(p=0.3)
        self._dropout_first = nn.Dropout(p=0.3)
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        x = self._dropout_first(x)
        x = self._fc_layer_age(x)
        x = self._dropout_last(x)
        x = self._act(x)
        x = self._batch(x)
        return self._fc_layer_last(x)
