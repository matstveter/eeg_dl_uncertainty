...
from eegDlUncertainty.data.results.history import History, TestHistory, get_history_objects
...
def objective(trial, fixed_params):
    ...
    #########################################################################################################
    age_scaling = "min_max"
    age_noise_level = trial.suggest_categorical("age_noise_level", [0.01, 0.05, 0.1, 1.5, 0.2, 0.25])
    clamp_age = trial.suggest_categorical("clamp_age", [True, False])
    clamp_val = (0, 1)

    # Adjust noise level based on scaling type
    conversion_rate = 7.38 if age_scaling == "sklearn_scale" else 1.0
    adjusted_age_noise_level = age_noise_level * conversion_rate

    age_noise_prob = 0.5
    earlystopping = 30
    print(f"Age scaling: {age_scaling}, Age noise level: {age_noise_level},"
          f"Adjusted_age_noise_level: {adjusted_age_noise_level} Clamp age: {clamp_age}")
    #########################################################################################################
    #########################################################################################################
    # MLFlow
    #########################################################################################################
    ...
    train_gen = CauDataGenerator(subjects=train_subjects, dataset=dataset, device=device, split="train",
                                 ...
                                 age_noise_prob=age_noise_prob, age_noise_level=adjusted_age_noise_level,
                                 clamp_age=clamp_age, clamp_val=clamp_val)
    val_gen = CauDataGenerator(subjects=val_subjects, dataset=dataset, device=device, split="val",
                               use_age=use_age)
    test_gen = CauDataGenerator(subjects=val_subjects, dataset=dataset, device=device, split="test",
                                use_age=use_age)
    ...
    try:
        ...
        optimizing_metric = test_hist.get_mcc()
...
def main():
    ...
    if args.config_path == "test_conf.json":
        ...
        experiment_name = f"optuna_augment_age_minmax"
