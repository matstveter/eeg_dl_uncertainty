...
def objective(trial, fixed_params):
    ...
    cnn_units = trial.suggest_int("cnn_units", 5, 20)
    max_kernel_size = trial.suggest_int("max_kernel_size", 5, 60, step=5)
    ...
    bagging_size = round(trial.suggest_float("bagging_size", 0.4, 0.9, step=0.1), 1)
    #########################################################################################################
    #########################################################################################################
    # MLFlow
    #########################################################################################################
    ...
    #########################################################################################################
    # Bagging specific operations
    #########################################################################################################
    trial_seed = random_state + trial.number
    random.seed(trial_seed)
    train_subjects = random.sample(train_subjects, int(bagging_size * len(train_subjects)))
    random.seed(random_state)
    #########################################################################################################
...
def main():
    ...
    if args.config_path == "test_conf.json":
        ...
        experiment_name = f"optuna_bagging"
