...
def main():
    ...
    with mlflow.start_run(run_name=folder_name):
        ...
        for run_id in range(num_runs):
            ...
            classifier = DummyModel(train_loader=train_loader)

            if use_test_set:
                evaluation_history = History(num_classes=dataset.num_classes, set_name="test",
                                             loader_lenght=len(test_loader), save_path=run_path)
                classifier.test_model(test_loader=test_loader, test_hist=evaluation_history, loss_fn=criterion)
            else:
                ...
                classifier.test_model(test_loader=val_loader, test_hist=evaluation_history, loss_fn=criterion)
